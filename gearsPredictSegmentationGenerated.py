...
path = Path("./GeneratedSyntheticGears/resized").rglob("*.png")#real images
...
for img_path in path:
    ...
    output = np.where(a['out'].cpu().detach().numpy()[0][0]>0.08,255,0) #selected threshold through visual analysis
    ...
    cv2.imwrite("./GeneratedSyntheticGears/mask/"+img_path.name,output)